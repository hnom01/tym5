...
public class ProfilController {
    ...
    @FXML private Label hesloMaskLabel;
    @FXML private TextField hesloVisibleField;
    @FXML private ToggleButton hesloToggleButton;
    @FXML private CheckBox emailNotifikacieCheckBox;
    @FXML private CheckBox statistickeUdajeCheckBox;
    @FXML private ToggleButton tmavyRezimToggle;
    ...
    private void ulozEmail() {
        if (aktualnyPouzivatelId < 0) return;
    ...
    private void otvorDialogZmenyHesla() {
        ...
        grid.add(new Label("Stare heslo:"), 0, 0); grid.add(stareHesloField, 1, 0);
        grid.add(new Label("Nove heslo:"), 0, 1); grid.add(noveHesloField, 1, 1);
        grid.add(new Label("Potvrdit heslo:"), 0, 2); grid.add(potvrditHesloField, 1, 2);
        ...
        if (result.isEmpty() || result.get() != ulozitButtonType) return;
        ...
        if (noveHeslo == null || noveHeslo.isBlank()) {
            zobrazChybu("Nove heslo nesmie byt prazdne."); return;
        }
        if (!noveHeslo.equals(potvrdenie)) {
            zobrazChybu("Nove hesla sa nezhoduju."); return;
        }

        boolean uspesne = DatabaseManager.zmenHeslo(aktualnyPouzivatelId, stareHeslo, noveHeslo);
        if (!uspesne) {
            zobrazChybu("Stare heslo nie je spravne alebo sa zmenu nepodarilo ulozit."); return;
        }
    ...
    private void odhlasSa(ActionEvent event) throws IOException {
        ...
        if (result.isEmpty() || result.get() != ButtonType.OK) {
            zobrazStatus("Odhlásenie zrušené."); return;
        }
        ...
        // PLYNULÝ PŘECHOD ZPĚT NA PŘIHLÁŠENÍ (zmenšení okna a vycentrování)
        stage.getScene().setRoot(root);
        stage.sizeToScene();
    ...
    private void nacitajProfil() {
        ...
        if (pouzivatel.isEmpty()) {
            zobrazStatus("Nie si prihlásený."); return;
        }
        ...
        if (topLevelLabel != null) topLevelLabel.setText("Level " + aktualny.getLevel());
        if (topXpLabel != null) topXpLabel.setText("XP: " + aktualny.getXp());
    ...
    private String vytvorMasku(String heslo) {
        if (heslo == null || heslo.isBlank()) return "********";
    ...
    private void obnovAktualnehoPouzivatela() {
        ...
        DatabaseManager.nacitajPouzivatelaPodlaId(aktualnyPouzivatelId).ifPresent(aktualny -> {
            ...
            aktualnyPouzivatelId = aktualny.getId();
            nacitajProfil(); // Aktualizujeme UI hned po obnove
        });
    }

    private void zobrazStatus(String sprava) {
        if (statusLabel != null) statusLabel.setText(sprava);
    }

    private void zobrazInfo(String sprava) {
        Alert alert = new Alert(Alert.AlertType.INFORMATION);
        alert.setTitle("Informacia"); alert.setHeaderText(null); alert.setContentText(sprava); alert.showAndWait();
    }

    private void zobrazChybu(String sprava) {
        Alert alert = new Alert(Alert.AlertType.ERROR);
        alert.setTitle("Chyba"); alert.setHeaderText(null); alert.setContentText(sprava); alert.showAndWait();
    }
}