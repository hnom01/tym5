...
public class ProfilController {
    ...
    @FXML
    private ToggleButton muzToggleButton;

    @FXML
    private ToggleButton zenaToggleButton;

    @FXML
    private ImageView avatarImageView;

    @FXML
    private Label pohlavieStatusLabel;
    ...
    private final ToggleGroup pohlavieToggleGroup = new ToggleGroup();

    @FXML
    private void initialize() {
        if (muzToggleButton != null) {
            muzToggleButton.setToggleGroup(pohlavieToggleGroup);
        }
        if (zenaToggleButton != null) {
    ...
    @FXML
    private void ulozPohlavie() {
        if (aktualnyPouzivatelId < 0) {
            return;
        }

        String pohlavie = getVybranePohlavie();
        AkciaVysledok vysledok = DatabaseManager.ulozPohlavie(aktualnyPouzivatelId, pohlavie);
        if (vysledok.uspesne()) {
            obnovAktualnehoPouzivatela();
            nacitajProfil();
            zobrazStatus(vysledok.sprava());
        } else {
    ...
    private void nastavPohlavie(String pohlavie) {
        String normalized = normalizujPohlavie(pohlavie);
        if (normalized == null) {
            if (pohlavieToggleGroup != null) {
                pohlavieToggleGroup.selectToggle(null);
            }
            if (pohlavieStatusLabel != null) {
                pohlavieStatusLabel.setText("Pohlavie nie je este nastavene.");
            }
            return;
        }

        if ("MUZ".equals(normalized)) {
            muzToggleButton.setSelected(true);
        } else if ("ZENA".equals(normalized)) {
            zenaToggleButton.setSelected(true);
        }

        if (pohlavieStatusLabel != null) {
            pohlavieStatusLabel.setText("Aktualne pohlavie: " + normalized.toLowerCase());
        }
    }

    private void aktualizujAvatar(String pohlavie) {
        if (avatarImageView == null) {
            return;
        }

        String normalized = normalizujPohlavie(pohlavie);
        String zdroj = switch (normalized == null ? "NEURCENE" : normalized) {
            case "MUZ" -> AVATAR_MUZ;
            case "ZENA" -> AVATAR_ZENA;
            default -> AVATAR_MUZ;
        };
        Image image = new Image(Objects.requireNonNull(getClass().getResource(zdroj)).toExternalForm());
        avatarImageView.setImage(image);
    }

    private String getVybranePohlavie() {
        if (muzToggleButton != null && muzToggleButton.isSelected()) {
            return "MUZ";
        }
        if (zenaToggleButton != null && zenaToggleButton.isSelected()) {
            return "ZENA";
        }
        return null;
    }

    private String normalizujPohlavie(String pohlavie) {
        if (pohlavie == null) {
            return null;
        }
